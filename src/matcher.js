...
// --- session guard --------------------------------------------------------
//
// Self-discipline, not security: anyone can open chrome://extensions and turn
// the whole thing off. The point is to add friction to an impulse, so the rules
// stay simple and honest about that.

/** Is the off-switch held shut right now? */
export function isLocked(settings, now = Date.now()) {
  const until = settings?.guard?.lockedUntil;
  return Boolean(settings?.enabled && until && until > now);
}

/** Milliseconds left on the lock, floored at 0. */
export function lockRemainingMs(settings, now = Date.now()) {
  return Math.max(0, (settings?.guard?.lockedUntil ?? 0) - now);
}

/** "4m 12s" — the countdown shown on the popup's disabled toggle. */
...
/** What you must type to break the lock early — your own topic or playlist name. */
export function unlockPhrase(settings) {
  const label = (settings?.mode === 'playlist' ? settings?.playlist?.title : settings?.topic?.label) || '';
  return label.trim() || 'tunneltube';
}

export function matchesUnlockPhrase(typed, settings) {
  return normalizeText(typed) === normalizeText(unlockPhrase(settings)) && Boolean(normalizeText(typed));
}

/** When switching on, how long the off-switch should stay shut. null = not at all. */
export function nextLockedUntil(settings, now = Date.now()) {
  const minutes = Number(settings?.guard?.minSessionMinutes ?? 0);
